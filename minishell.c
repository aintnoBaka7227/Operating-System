  ...
  */
  ...
  while (1) {			/* do Forever */
    ...
    if (v[0] && strcmp("cd", v[0]) == 0) {
        ...
        if (v[1] == NULL) {
            target = getenv("HOME");
            // go to / if HOME is not set up
            if (target == NULL) {
                target = "/";
            }
        } else {
          // if path is included go to path
          target = v[1]; 
        }
        if (chdir(target) == -1) {
            perror("chdir");
    ...
    switch (frkRtnVal = fork()) {
      ...
      {
        // store child process job 
        if (bg_flag) {
            // store pid id
            bg_pid_nums[count] = frkRtnVal;
            // store command line for background job execution
          ...
          // forground wait for child process in background finish execution
        ...
        for (int job = 1; job < count; job++) {
            if (bg_pid_nums[job] > 0) {
              // parent collect finish status from background job
                pid_t result = waitpid(bg_pid_nums[job], NULL, WNOHANG);
                if (result == bg_pid_nums[job]) {
                    printf("[%d]+ Done %s\n", job, bg_pid_args[job]);
                    // reset active status to non 
                    bg_pid_nums[job] = 0;
                }
            }
        }
